...
function meteor(attr) {
  ...
  var checkedPos = function(upperb, lowerb, rightb, leftb){
	if(x > rightb || x < leftb || y < upperb || y > lowerb){
		x = 0;
		y = 0;
...
var Manager = (function() {
  ...
  var parseNearbyPlanet = function() {
    ...
    var leftbound = 0 - Ncontext.canvas.width * 5;
    var rightbound = 0 + Ncontext.canvas.width * 5;
    var upperbound = 0 - Ncontext.canvas.height * 5;
    var lowerbound = 0 + Ncontext.canvas.height * 5;
    for (var i = 0; i < len; i++) {
      ent = starList[i];
  ...
  var checkBound = function() {
    ...
    if (test1.x > 0 || test1.y > 0 || test2.x < screenSize.width || test2.y < screenSize.height){
		reachBound = true;
  ...
  var gameloop = function() {
    ...
    if (!PersonalMode) {
      ...
      for (var i = 0; i < meteorLen; i++) {
        ...
        //get acceleration
        ent.update(dTime);
		//checked if out range
		ent.checkedPos(upperBound, lowerBound, rightBound, leftBound);
        if (ent.draw) {
          ent.draw(Ncontext, Camera);
        }
      }
...
var ButtonHandler = (function() {
  var init = function() {
	...
	$("#search-normal").click(function(){
		...
		if(content){
			Camera.startMove(		Manager.searchPlanet(content), false);
		}
	...
	$("#search-proper").click(function(){
		var content = $("#searchContent").val();
		if(content){
			Camera.startMove(		Manager.searchPlanet(content), true);
		}
	});
...
var ImageLoader = (function(){
	var rawList = [];
	var imageList = [];
	var nameList = ['test', 'background', 'gold', 'soil', 'bronde', 'wood', 'water'];
	var loadedAmount = 0;
	var loadedMax = 7; //the total of images
